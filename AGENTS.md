...
1. 将分钟行情聚合为日频行情。
2. 通过独立的因子工厂生成特征。
3. 使用当日及以前可见的数据预测可配置目标的收益率是否严格大于标签阈值（默认 0.5%，可配置）或连续涨跌幅：下一交易日收盘至再下一交易日收盘（`close`）、下一交易日开盘至再下一交易日开盘（`open`），或下一交易日开盘至收盘（`inday`）；训练时可按目标买卖日的成交约束剔除明确无法买入或卖出的历史样本，但验证预测截面保持完整。
...
- `set_fit_progress(callback)` 是可选能力，用于在一次 `fit` 内部按迭代轮上报进度：
  实现它的模型在训练中按轮调用 `callback(已完成轮数, 总轮数)`，并返回本次训练预计的
  总轮数，传入 `None` 表示卸载；无法预知轮数时返回 `None`。未实现该方法的模型不受
  影响，整段训练在进度条上只占一步。该能力只用于显示，**不得改变训练结果**，因此
  装卸回调前后的预测必须逐点一致（见
  `tests/factor_research/test_lightgbm_model.py` 的对照用例）。
