...
def run_top_n_intraday_backtest(
    ...
    """回测每日按预测分数选股并实现配置目标收益的等权策略。

    每个目标交易日在全部有效预测中按分数降序、证券代码升序稳定选取最多
    ``top_n`` 只。买入成交价相对目标起始价上浮滑点，卖出成交价相对目标结束价
    下调滑点，手续费在买卖两边各收取一次。收益曲线从 1.0 开始按每日净收益
    复利。夏普比率采用零无风险利率、样本标准差和 252 日年化。

    参数：
        predictions: 验证集逐证券预测，须含目标日期、证券代码、配置口径的
