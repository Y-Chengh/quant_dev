...
每个业务分区的 `_SUCCESS.json` 包含完成时间、行数、`job_key`、`mode`、CSV 的 SHA-256，以及记录当次证券池和数据集列表的 `partition_scope`。写入顺序为 `data.csv.tmp` → 原子替换 `data.csv` → `_SUCCESS.json.tmp` → 原子替换 `_SUCCESS.json`。增量模式默认跳过同一证券池的已有完成分区，因此不会重写历史文件；若同一输出目录和日期改用了不同证券池，任务会明确报错，需使用 `repair` 或新输出目录，避免静默丢股票。

`partition_scope` 只在 `datasets` 选中 `finance_raw` 或 `finance_daily` 时，才额外记录 `finance_lookback_start` 与财务字段清单：不下载财务数据时这两项对 `kline_1d`、`corporate_actions` 分区没有任何影响，写进去只会让改一次财务参数就把已有分区判为口径不符（处理原则与 `save_instrument_history` 一致）。比较双方在核验前都会剔除这种无效的财务键，因此旧版本写出的分区带着这两项也能继续匹配，既有输出目录不需要 `repair` 或重建。证券池、数据集列表，以及下载财务数据时的财务口径，仍然严格全等比较。兼容只保证这一个方向：若把库代码回滚到本次修复之前的版本，新写出的不带财务键的分区会被旧代码判为口径不符并中断任务（报错文本仍是“分区已由不同证券池或数据范围写入”，容易误导向证券池方向排查），此时需要一并回滚或改用新输出目录。
