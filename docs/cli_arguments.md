...
于滑点。报告里会多出「不同滑点下的收益曲线对比」小节与 `_slippage.svg` 图，基准
行即 `--slippage-bps` 的取值，其余指标（回撤、基准对照、十分位等）不受影响。
...
- 进度粒度随训练方式而定：`rolling` 按验证交易日推进；`single` 下若模型支持逐轮上报（当前是 `lightgbm`），训练段按提升轮细分为 `n_estimators` 步并显示「训练 168/500 轮」，否则整段训练只占一步，仍是预处理、训练、预测三步。
