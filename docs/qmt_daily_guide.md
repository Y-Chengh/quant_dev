...
命令结尾的摘要会列出全部七个过滤原因各自丢了多少行（没丢的显式写 `0 行`），并给
非零原因附最多 10 条具体样例，方便确认滤掉的确实该滤。判定条件见
[qmt_daily_store.md](qmt_daily_store.md) 的「过滤原因与样例」。其中最该盯的是
`before_listing_with_data`：**它正常应该是 0**，非 0 说明上市日或行情归属出了问题。
