...
命令结尾的摘要会列出全部七个过滤原因各自丢了多少行（没丢的显式写 `0 行`），每个原因
下面附判定条件、含义和处理建议，非零原因再给最多 10 条具体样例。**只看过滤小节末尾
那行结论就够**（它后面可能还跟着「待定分区」与「日志已保存」两类行）：

```
本次过滤全部落在预期原因内，无需人工核对。
```

七个原因里只有 `before_listing_padding`（QMT 给未上市证券造的占位行）非 0 属正常，
其余六个正常都应为 0。命中任何一个，末行会变成 `需人工核对: <原因>(<行数> 行)`，
照着该原因下面的「处理」那行做即可。判定条件详见
[qmt_daily_store.md](qmt_daily_store.md) 的「过滤原因与样例」。
